...
-- Have 4 resources for each user (16 total):
-- One of those 16 resources should be named “Leo’s Ladder” and other attributes set to anything of your choosing.
-- One of those 16 resources should have the word “Ladder” in the model and the name “Will’s Little Giant” and other attributes set to anything of your choosing.
...
-- The other 13 resources can be created with attributes of your choosing, ensuring that there is at least one resource for each ESF in the database.  (Since there are 16 resources we are asking for, and 15 ESFs, you are going to have at least one ESF with two resources.)
-- Have two of each incident type already populated in the database.  These incidents can have attributes of your choosing.  (You could have all of one incident type assigned to each user, but the distribution of these eight incident types is entirely up to you.)
-- One resource from each user in your database should be in use until 7/30/2018, and must have been requested by a different user.
-- One resource request for each user in your database should be pending.
